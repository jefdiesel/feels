    ...
    try {
      ...
      if (token) {
        ...
        try {
          ...
          console.log(`[API] ${config.method?.toUpperCase()} ${config.url} (user: ${payload.user_id?.substring(0, 8)}...)`);
        } catch {
          console.log(`[API] ${config.method?.toUpperCase()} ${config.url} (has token)`);
        }
      } else {
      ...
      console.log(`[API] ${config.method?.toUpperCase()} ${config.url} (storage error)`);
    }
    return config;
  },
  (error) => {
...
export const authApi = {
  ...
  // Sign in with Apple
  appleAuth: (identityToken: string, userId: string, email: string | null, fullName: string | null, deviceId: string, platform: string) =>
    api.post<{ access_token: string; refresh_token: string; expires_in: number; is_new_user: boolean }>(
      '/auth/apple',
      { identity_token: identityToken, user_id: userId, email, full_name: fullName, device_id: deviceId, platform }
    ),
...
export const profileApi = {
  ...
  updatePreferences: (data: {
    ...
    is_private?: boolean;
    gender_presentations?: Record<string, { enabled?: boolean; bio?: string; age_min?: number; age_max?: number }>;
