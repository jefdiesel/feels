...
export function useRevenueCat(): UseRevenueCatReturn {
  ...
  const initialize = useCallback(async (userId?: string) => {
    ...
    try {
      ...
      // Get initial customer info - wrap in try/catch since this can fail with test keys
      ...
      try {
        ...
        console.warn('[RevenueCat] Could not get customer info:', e);
      }

      // Get offerings - also wrap since it may fail
      let offerings = null;
      try {
        const off = await Purchases.getOfferings();
        offerings = off.current;
      } catch (e) {
        console.warn('[RevenueCat] Could not get offerings:', e);
      }
      ...
      // Don't crash - just mark as ready with no premium
      setState((prev) => ({
        ...prev,
        isReady: true,
        isPremium: false,
